...
import { RolesGuard } from './authentication/auth/guards/role.guard';
import { AdminGuard, RtGuard } from './authentication/auth/guards';
...
async function bootstrap() {
  ...
  app.useGlobalGuards(new AdminGuard(reflector), new RolesGuard(reflector));
  app.use(function (
  ...
  const config = new DocumentBuilder()
    .setTitle('IMS')
    .setDescription('Institution Management System Backend API')
    .setVersion('1.0')
    .setLicense('IMS', 'https://ims.com/')
    .addBearerAuth(
      {
        type: 'http',
        scheme: 'bearer',
